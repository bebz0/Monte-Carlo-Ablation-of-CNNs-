...
if __name__ == '__main__':
    ...
    # --- Ablation targets: all nn.Conv2d in features ---
    conv_targets = [
        (i, layer)
        for i, layer in enumerate(model.features)
        if isinstance(layer, nn.Conv2d)
    ]
    total_conv = len(conv_targets)
    conv_indices = [i for i, _ in conv_targets]
    conv_layers  = [l for _, l in conv_targets]
    print(f"\nConv layers: {total_conv} | Indices: {conv_indices}")
    ...
    print(f"\n=== Monte Carlo: {NUM_TRIALS} trials × {total_conv} levels ===")
    t_start = time.time()

    pbar = tqdm(total=total_conv * NUM_TRIALS, unit="eval")

    for num_to_disable in range(1, total_conv + 1):
        for trial in range(1, NUM_TRIALS + 1):

            trial_seed = 42 + num_to_disable * 1000 + trial
            random.seed(trial_seed)

            chosen_layers = random.sample(conv_layers, num_to_disable)

            handles = [l.register_forward_hook(zero_out_hook) for l in chosen_layers]
            ...
            results.append({
                ...
                "Disabled_Percentage": round(num_to_disable / total_conv * 100, 4),
                "Trial": trial,
                "Accuracy": acc,
            })
    ...
    save_path = f"vgg16_mc_{NUM_TRIALS}trials.csv"
